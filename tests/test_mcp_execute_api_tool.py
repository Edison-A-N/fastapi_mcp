...
@pytest.mark.asyncio
async def test_execute_api_tool_with_structured_content(simple_fastapi_app: FastAPI):
    """Test execution of an API tool with structured content enabled."""
    mcp = FastApiMCP(simple_fastapi_app, prefer_structured_content=True)
    ...
    # Verify the result is structured content (dict)
...
@pytest.mark.asyncio
async def test_execute_api_tool_with_structured_content_list(simple_fastapi_app: FastAPI):
    """Test execution of an API tool returning a list with structured content enabled."""
    mcp = FastApiMCP(simple_fastapi_app, prefer_structured_content=True)
    ...
    # Verify the result is structured content (list)
...
@pytest.mark.asyncio
async def test_execute_api_tool_with_structured_content_disabled(simple_fastapi_app: FastAPI):
    """Test execution of an API tool with structured content disabled (default behavior)."""
    mcp = FastApiMCP(simple_fastapi_app, prefer_structured_content=False)
    ...
    # Verify the result is text content (default behavior)
    assert len(result) == 1
    assert isinstance(result[0], TextContent)
    assert result[0].text == '{\n  "id": 1,\n  "name": "Test Item"\n}'
...
@pytest.mark.asyncio
async def test_execute_api_tool_with_non_json_response_structured_content(simple_fastapi_app: FastAPI):
    """Test execution of an API tool with non-JSON response when structured content is enabled."""
    mcp = FastApiMCP(simple_fastapi_app, prefer_structured_content=True)

    # Mock the HTTP client response with non-JSON content
    mock_response = MagicMock()
    mock_response.status_code = 200
    ...
    # Verify the result is text content (fallback for non-JSON)
    assert len(result) == 1
    assert isinstance(result[0], TextContent)
    assert result[0].text == "Plain text response"
